# Sordi API (web backend)

Node/Express/Postgres backend for the web version of Sordi, now `apps/api` in the pnpm workspace. Its Drizzle schema lives in `packages/schema` (shared, not duplicated) — the desktop app (`src/`, `src-tauri/`) doesn't depend on anything here and is unaffected by it.

Current scope: auth (signup/login/refresh/logout) + `clients` + `products`, the first vertical slice proving the multi-tenant pattern end-to-end. Every other resource (invoices, payments, orders, delivery notes, expenses, dashboard, production, employees, projects, scores, history) follows the exact same pattern — `packages/schema` already covers all of them; only the repository + routes are still to come.

## Run it locally

`pnpm install` is run once from the **repo root** (installs every workspace member, this one included) — not from inside this directory.
...
pnpm run db:generate          # generates SQL migrations from packages/schema/src/schema.ts
pnpm run db:migrate           # applies them
pnpm run dev                  # API on http://localhost:4000
```
...
# one-time setup — creates apps/api/.pgdata/ (gitignored)
initdb -D "$(pwd)/.pgdata" -U sordi --auth=trust -E UTF8
mkdir -p /tmp/sordi_dev_sock

# start/stop the local instance (port 5434, matches .env.example)
pg_ctl -D "$(pwd)/.pgdata" -l "$(pwd)/pg.log" -o "-p 5434 -k /tmp/sordi_dev_sock" start
createdb -h /tmp/sordi_dev_sock -p 5434 -U sordi sordi   # first time only
