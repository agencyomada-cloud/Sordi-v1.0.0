...
pub fn init_database(db_path: &str) -> Result<Connection, rusqlite::Error> {
    ...
    // Activités & Rappels (Odoo "chatter" style follow-ups) — a lightweight
    // task attached to any record (invoice, client, supplier, quote). No
    // FOREIGN KEY on entity_id: entity_type spans multiple tables, so it's
    // a polymorphic reference resolved in the app layer, same convention
    // as activity_logs.entity_type/entity_id below.
    conn.execute(
    ...
    // with zero indexes previously defined, all of them were full table
    // scans. Created last, after every migration above, so every column
    // referenced (company_id included, added by migrate_multi_company_if_needed
    // just above) is guaranteed to already exist. `CREATE INDEX IF NOT
    // EXISTS` makes this idempotent on every existing database, exactly
    // like the ALTER-TABLE migrations above.
    //
    // Credit notes ("avoirs") have no separate table — they're rows in
    // `invoices` with invoice_type = 'credit_note', so their client_id
    // lookup is already covered by idx_invoices_client_id, and
    // idx_invoices_original_invoice_id covers the "which invoice does this
    // credit note reverse" lookup.
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_invoices_invoice_date ON invoices(invoice_date)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_invoices_created_at ON invoices(created_at)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_invoices_original_invoice_id ON invoices(original_invoice_id)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_invoices_due_date ON invoices(due_date)", [])?;
    // Composite index for dashboard/report queries filtering by status
    // within a date range (e.g. "unpaid invoices issued this year").
    conn.execute("CREATE INDEX IF NOT EXISTS idx_invoices_status_invoice_date ON invoices(status, invoice_date)", [])?;
    // Every list/dashboard query filters on company_id first (multi-company
    // isolation) before anything else — this composite lets that filter,
    // plus a status/date-ordered scan on top of it, both use one index via
    // its leftmost-prefix columns instead of a full scan per company.
    conn.execute("CREATE INDEX IF NOT EXISTS idx_invoices_company_status_date ON invoices(company_id, status, invoice_date)", [])?;
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_expenses_expense_date ON expenses(expense_date)", [])?;

    conn.execute("CREATE INDEX IF NOT EXISTS idx_clients_name ON clients(name)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_clients_rc ON clients(rc)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_clients_nif ON clients(nif)", [])?;

    // Covers list_activities' WHERE entity_type = ? AND entity_id = ?
    // filter plus its ORDER BY due_date, all from one index.
    conn.execute("CREATE INDEX IF NOT EXISTS idx_activities_entity ON activities(entity_type, entity_id, due_date)", [])?;
...
fn migrate_companies_extra_fields_if_needed(conn: &Connection) -> Result<(), rusqlite::Error> {
    let table_info: Result<Vec<_>, _> = conn.prepare("PRAGMA table_info(companies)")?
        ...
        .collect();
    if let Ok(column_names) = table_info {
        for column in ["phones", "website", "capital", "rib", "bank_agency", "extra_info", "cnas_adherent", "legal_form"] {
