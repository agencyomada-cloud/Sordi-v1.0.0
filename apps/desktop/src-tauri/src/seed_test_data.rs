...
//! ~/Library/Application Support/com.sordi.app/database.db). Override with
...
fn resolve_db_path() -> std::path::PathBuf {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        std::path::PathBuf::from(home).join("Library/Application Support/com.sordi.app/database.db")
    }
    #[cfg(target_os = "windows")]
    {
        let appdata = std::env::var("APPDATA").expect("APPDATA not set");
        std::path::PathBuf::from(appdata).join("com.sordi.app/database.db")
    }
    #[cfg(target_os = "linux")]
    {
        let home = std::env::var("HOME").expect("HOME not set");
        std::path::PathBuf::from(home).join(".local/share/com.sordi.app/database.db")
    }
