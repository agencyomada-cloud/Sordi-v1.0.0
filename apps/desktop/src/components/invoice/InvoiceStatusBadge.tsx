 ...
 * it can't pollute an exported PDF.
 *
...
export function InvoiceStatusBadge({ status, className }: { status: string | null | undefined; className?: string }) {
  ...
  return (
    <div
      className={cn(
        "absolute top-3 right-3 z-20 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wide border shadow-sm select-none pointer-events-none",
