...
export function OrderEditableEpure({ order, clients, settings, logic, readOnly = false }: Props) {
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[9pt] text-gray-500 mt-1 flex items-center justify-end gap-1">
                      ...
                      {readOnly ? <span>{order.order_date}</span> : (
                        <Input type="date" value={order.order_date || ""} onChange={(e) => updateOrderField('order_date', e.target.value)} className="h-5 w-28 border-none bg-transparent p-0 text-right shadow-none focus-visible:ring-0 text-[9pt]" />
                      )}
                    </div>
                  </div>
                </div>
