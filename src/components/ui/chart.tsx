...
/**
 * Sanitizes a CSS value to prevent style injection attacks.
 * Only allows valid CSS color values (hex, rgb, hsl, named colors).
 * 
 * SECURITY: Chart config must come from developer-defined configuration only,
 * never from user-controlled input. This sanitization is a defense-in-depth measure.
 */
const sanitizeCssValue = (value: string): string | null => {
  if (!value || typeof value !== 'string') return null;
  
  // Allow only valid CSS color patterns
  const validPatterns = [
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Build CSS variables safely using React's style prop instead of dangerouslySetInnerHTML
  // This approach is safer as React handles escaping automatically
  const cssVariables = React.useMemo(() => {
    ...
    colorConfig.forEach(([key, itemConfig]) => {
      ...
      // For each theme, we'd need the appropriate color
      // Default to the base color for CSS variable fallback
  ...
  // Generate CSS string with sanitized values for theme support
  const cssString = React.useMemo(() => {
    return Object.entries(THEMES)
      .map(([theme, prefix]) => {
        ...
        // Sanitize id to prevent selector injection
